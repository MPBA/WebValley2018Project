...
import dp_optimizer, dp_pca, sanitizer, utils, accountant
...
tf.flags.DEFINE_integer("projection_dimensions", 0,
...
tf.flags.DEFINE_integer("num_conv_layers", 2,
...
tf.flags.DEFINE_float("eps", 0, #1.0,
...
tf.flags.DEFINE_float("sigma", 0, #4.0,
...
tf.flags.DEFINE_string("save_path", "/home/user/checkpoints",
...
NUM_TRAINING_IMAGES = dataset.X_train_input.shape[0]
NUM_TESTING_IMAGES = dataset.X_dev_input.shape[0]
IMAGE_SIZE = 28


def BRATSInput(is_train=True, batch_size=100):
    global dataset
    if is_train:
        X = dataset.X_train_input
        y = dataset.y_train_target
    else:
        X = dataset.X_dev_input
        y = dataset.y_dev_target

    X = tf.constant(X)
    y = tf.constant(y)
    # print(X.shape)
    # print(y.shape)
    # images = tf.train.batch({'images': X}, batch_size=batch_size)
    # labels = tf.train.batch({'labels': y}, batch_size=batch_size)
...
def Eval(mnist_data_file, network_parameters, num_testing_images,
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        images, labels = BRATSInput(is_train=False)
...
def Train(mnist_train_file, mnist_test_file, network_parameters, num_steps,
    ...
    with tf.Graph().as_default(), tf.Session() as sess, tf.device('/cpu:0'):
        ...
        cost = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=tf.one_hot(labels, 2))
